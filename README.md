...
<p align="center">
  ...
  ·
  <a href="星港拾荒者-v0.1.0-v0.28.0-Patch-Notes.txt">完整更新记录</a>
  ·
...
- **研究与星核**：沿四条双分支科技树建立 24 项研究，深空跃迁后使用星核获得永久强化。
...
- **排行榜**：登录后可比较最高自动产量、最高战力、研究与星港峰值、战斗远征、奇点超越和边境星区。
- **航站通讯**：在游戏内阅读运营公告；登录后可直接提交问题、建议与联系方式。
- **航路共鸣**：八章渐进式目标把现有系统串成一条清晰成长线；33 项图鉴、每日机制首领与共同航标提供收藏、策略和合作目标。

## 当前版本

当前航站协议为 **v0.28.0**。

- v0.28.0：远征页新增每周三选一深空异象，七种异常连接现有玩法、提供有限收益与风险，并用纯收藏档案替代新的永久倍率。

- v0.27.0：新增三种跃迁学说，每次深空跃迁后三选一，以有限收益与明确代价改变当前航线玩法，并在下次跃迁时重选。

- v0.26.0：新增归航简报、每日三选一值守目标和仅随个人存档保存的匿名体验里程碑，让玩家回来后立即知道获得了什么、现在做什么。

- v0.25.0：新增八章新手航路、33 项星海图鉴、每日三阶段机制首领与全服共同航标，四项内容均并入现有页面且不增加新货币。
...
- v0.21.0：新增四首原创 BGM 的自动轮播与单曲选择，并为每首音乐独立优化首尾静音衔接。
...
完整版本变化见 [Patch Notes](星港拾荒者-v0.1.0-v0.28.0-Patch-Notes.txt)。
...
云端配置说明见 [FIREBASE-SETUP.txt](FIREBASE-SETUP.txt)，公告发布与反馈处理流程见 [COMMUNICATIONS-GUIDE.txt](COMMUNICATIONS-GUIDE.txt)，换电脑继续开发请先阅读 [新电脑维护交接说明-v0.28.0.md](新电脑维护交接说明-v0.28.0.md)。
