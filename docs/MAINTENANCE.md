...
- `assets/fleet-hangar.webp` / `.fleet-illustration`：舰队整备机库场景。
- `assets/research-observatory.webp` / `.research-illustration`：深空研究观测室场景。
- `assets/orbital-defense.webp` / `.combat-illustration`：轨道防卫与袭击场景。
- `assets/singularity-transcend.webp` / `.transcend-illustration`：奇点超越与伴星信标场景。
- `.page-illustration`：四张页面插图共用的遮罩、说明文字与手机裁切规则。

## v1.9.0 关键入口

- `STARPORT_LIFE_EVENTS` / `renderStarportLife()`：六段每日生活片段、五组收藏统计与现有资源奖励。
- `hydrateAnnouncementGoals()`：根据公告字段通过 Firestore 聚合读取排行榜全服进度。
- `assets/starport-gallery.webp` / `.starport-life`：陈列廊主题贴图、收藏概览与手机单列日常事件。
- `renderReturnProtocol()`：归航时补充待领委托、收藏进度和当前长航或蓝图状态。

- `LONG_VOYAGE_CHOICES` / `LONG_VOYAGE_EVENTS`：三种阶段应对、四种普通信号与三种机制型终点。
- `chooseLongVoyageDecision()` / `quickSettleLongVoyageStage()`：阶段计数起点、机制判断与归档路线资源回收。
- `assets/voyage-crossroads.webp` / `.long-voyage-decision`：长航主题贴图与响应式决策卡。
