...
async function main() {
  ...
  await context.addInitScript((legacySave) => {
    ...
    localStorage.setItem("stellarOutpostIdlePatchNotesSeen", "1.5.0");
  ...
  try {
    ...
    const snapshot = await page.evaluate(() => ({
      ...
      rebuild: window.StellarOutpostCloudBridge.getRebuildDiagnostics(),
      companionEchoes: window.StellarOutpostCloudBridge.getCompanionEchoDiagnostics(),
      longVoyage: window.StellarOutpostCloudBridge.getLongVoyageDiagnostics(),
      ...
      currentActionVisible: getComputedStyle(document.querySelector("#command-guide")).display !== "none",
      statRows: document.querySelectorAll("#stat-breakdown-list article").length,
      atlasNextTitle: document.querySelector("#atlas-next-title").textContent,
      collapsePreview: document.querySelector("#collapse-recovery-estimate").textContent,
      mobileNavigationItems: document.querySelectorAll("#mobile-quick-nav button").length,
    }));

    assert.equal(snapshot.gameVersion, "1.5.0");
    ...
    assert.match(snapshot.footer, /v1\.5\.0/);
    ...
    assert.equal(snapshot.rebuild.state.plans.length, 3);
    assert.equal(snapshot.rebuild.state.activePlanId, "");
    assert.equal(snapshot.companionEchoes.echoes.length, 8);
    assert.equal(snapshot.companionEchoes.records.length, 0);
    assert.equal(snapshot.longVoyage.routes.length, 3);
    assert.equal(snapshot.longVoyage.active, null);
    ...
    await page.locator("#focus-route-list [data-focus-pin]").first().click();
    assert.equal(await page.locator("#tracked-goals").isVisible(), true);
    assert.equal(await page.locator("#tracked-goal-list .tracked-goal").count(), 1);
    await page.locator("#operations-hub > summary").click();
    await page.waitForTimeout(220);
    for (let modalAttempt = 0; modalAttempt < 3; modalAttempt += 1) {
      if (!(await page.locator("#modal-confirm").isVisible())) break;
    ...
    assert.ok(rememberedOperation.lastJobId);
    assert.equal(rememberedOperation.queue[0].jobId, rememberedOperation.lastJobId);
    assert.equal(await page.locator("#operations-repeat-button").isEnabled(), true);
    await page.locator("#operations-stop-button").click();
    await page.locator("#operations-repeat-button").click();
    assert.equal(
      await page.evaluate(() => window.StellarOutpostCloudBridge.createSnapshot().operations.queue[0].jobId),
      rememberedOperation.lastJobId,
    );
    ...
    for (let observationAttempt = 0; observationAttempt < 8; observationAttempt += 1) {
      const resolved = await page.evaluate(() => {
        document.querySelector('[data-companion-event-choice="archive"]')?.click();
        return window.StellarOutpostCloudBridge.createSnapshot()
          .endgame.companionObservations.length > 0;
      });
      if (resolved) break;
    ...
    assert.ok(await page.locator("#companion-echo-list .companion-echo-card").count() >= 1);
    ...
    assert.equal(startedLongVoyage.active.id, "industrial");
    assert.equal(startedLongVoyage.state.stageIndex, 0);
    assert.equal(await page.locator("#long-voyage-active").isVisible(), true);
    ...
    const missionClaimButton = page.locator("#claim-all-missions-button");
    await page.waitForFunction(() => {
      const button = document.querySelector("#claim-all-missions-button");
    ...
    await page.route("**/index.html?check=*", (route) =>
      route.fulfill({
        ...
        body: '<!doctype html><meta name="stellar-game-version" content="1.5.1"><meta name="stellar-release-title" content="更新检测测试">',
      }),
    );
    await page.evaluate(() =>
    ...
    assert.match(await page.locator("#update-banner-title").textContent(), /v1\.5\.1/);
